...
image=cv2.imread(r"images/white_pilss.png")# resim okundu
...
#### üstteki for döngüsü saptanan her kontura numara verdiğinden göz ardı edeceğimiz konturlarda birer numara alır ve bu göstermememiz gereken nesneleri sıraya alır bunun için döngünün içinde koşulları sağlayan nesneler için bir numara oluşturdum
i=0
for (c) in cnts: 
    ...
    if cv2.contourArea(c)<100:# küçük konturları göz ardı etmek için koşul yazıldı
       ...
       continue
    i=i+1# göz ardı ettiğimiz nesneleri saymamak için döngüye sayaç eklendi
    ...
    print(f"Object # {i}")# orijinal kordinatlar gösterildi
    ...
    cv2.putText(image,f"Object #{i}",(int(rect[0][0]-15),int(rect[0][1]-15)),cv2.FONT_HERSHEY_SIMPLEX,0.55,(255,255,255),2)#nesnelerin numaraların yazılacağı yerler konumları ile beraber verildi ve yazıldı    
